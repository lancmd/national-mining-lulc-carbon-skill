...
标准 InVEST 按地类平均面积碳密度计算，不能直接表达水体库容变化。本模块在数据充分时，将沉陷积水拆分为水体碳、水生植被碳和底泥碳。
...
若已有测深 DEM 或可靠的沉陷后地形和水面高程，可逐像元计算：

```text
depth_m = max(water_surface_elevation - bed_elevation, 0)
...
底泥面积应来自测量、沉积范围解释或明确假设。不能默认等于水体面积；论文案例中底泥覆盖面积甚至可与某期水面植被面积不同，必须独立记录。
...
论文五沟煤矿案例报告：水体碳密度 26.25 g C/m³、水生植被碳密度 0.46 t C/hm²、底泥碳密度 62.42 t C/hm²。这些值仅用于理解公式或复现论文，不能作为其他矿区默认值。
